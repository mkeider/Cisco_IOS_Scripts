...
#################################################################################################
#Script connects to cisco_ios device extracts IP OSPF Interface data, parses with textfsm, then #
#outputs to .csv "OSPF_Interface.csv"                                                           #
#################################################################################################
...
for device in devices:
    ...
    try:
        ...
        print(local_host)
        output = connection.send_command("show ip ospf interface brief", use_textfsm=True)##textfsm integrated into Netmiko

        for item in output:
            item.update({"local_host":local_host})#Insert Local Hostname into dictionary
            ospf_results.append(item) #For Loop to append the OUTPUT results from connection.send_command
...
with open('ospf_connection_results.json', 'w') as results_file:
    json.dump(results, results_file, indent=2)


with open('ospf_int_brief.csv','w', newline='')as new_file:
   fieldnames = ['local_host','interface','area','ip_address_mask', 'cost', 'state', 'neighbors_fc']
   csv_writer = csv.DictWriter(new_file, fieldnames=fieldnames, delimiter=',')

   csv_writer.writeheader()

   for line in (ospf_results):
