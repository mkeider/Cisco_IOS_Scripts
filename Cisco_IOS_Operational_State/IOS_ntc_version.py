...
#################################################################################################
#Script connects to cisco_ios device extracts IOS Version data, parses with textfsm, then       #
#outputs to .csv "IOS_Version.csv"                                                              #
#################################################################################################
...
for device in devices:
    ...
    try:
        ...
        print(local_host)
        output = connection.send_command("show version", use_textfsm=True)##textfsm integrated into Netmiko

        for item in output:
         version_results.append(item) #For Loop to append the OUTPUT results from connection.send_command
...
with open('version_results.json', 'w') as results_file:
    json.dump(results, results_file, indent=2)


with open('version.csv','w', newline='')as new_file:
   fieldnames = ['hostname','version','rommon','uptime','running_image', 'hardware', 'serial','config_register']
   csv_writer = csv.DictWriter(new_file, fieldnames=fieldnames, delimiter=',')

   csv_writer.writeheader()

   for line in (version_results):
